...
def main():
    ...
    if "LoRA" in config.PM_path:
        ...
        preference_model = AutoModelForSequenceClassification.from_pretrained(peft_config.model_name)
        preference_model = PeftModel(preference_model, config)
    else:
        preference_model = AutoModelForSequenceClassification.from_pretrained(config.PM_path)
    preference_model.eval()
    # Load the tokenizer for the preference model
    preference_tokenizer = AutoTokenizer.from_pretrained(config.PM_path, use_fast=True)
    ...
    base_model = AutoModelForCausalLMWithValueHead.from_pretrained(config.base_model, peft_config=peft_config)
